...
function UpdatePostBtgv(props) {
    ...
    const fetchData = () => {
        axios.get(`/api/blog1/getBlogs/${postId}`)
            .then(response => {
                if (response.data) {
    ...
    const onSubmit = (event) => {
        ...
        // axios.get(`http://localhost:5000/api/blog1/getBlogs/${postId}`)
        //     .then(response => {
        //         if (response.data) {
        //             setContent(response.data.content)
        //             setTitleValue(response.data.title)
        ...
        if(TitleValue!==''){
            ...
            axios.put(`/api/blog1/getBlogs/${postId}`, variables)
                .then(response => {
                    if (response) {
